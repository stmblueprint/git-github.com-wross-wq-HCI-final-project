...
function fetchMicrosoft($conn){
 $sql = "SELECT m.title, m.video, m.image, m.price m.console FROM Microsoft m WHERE m.title = 'xbox'";
          ...
          while($row = mysqli_fetch_array($result)){
              echo "
               ...
               <div class='item-container'>
                <div class='item-img'>
                <a href='product-page.php?id=".$row[0]."&title=".$row[1]."&video=".$row[3]."&image=".$row[4]."&price=".$row[5]."&console=xbox'><img src=".$row[4]." width='140' height='190'></a></div>
               </td>
               </div>";
          }
        echo "</tr></table>";
